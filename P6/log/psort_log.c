...
/* Step 1: parse the input into an array*/
  ...
  {
    lines[i] = malloc(sizeof(keyValuePair) * LINE_LENGTH);
    lines[i]->value = malloc(sizeof(char) * LINE_LENGTH);
  }
...
/* Step 4: create a final array that integrates the segregated arrays*/
  ...
  // final array to sort all lines in 2 threads/arrays
  keyValuePair *final_array[num_lines];

  // copy the child array to the final array
  j = 0;
  k = 0;
  for(int i = 0; i < num_lines; i++){
